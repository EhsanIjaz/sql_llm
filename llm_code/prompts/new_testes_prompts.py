...
use SUM() instead of MAX() not in the case of COUNT()
**Rules***

- Always filter queries by month and year if not specified in the question.data is from month july 2024 to december 2024 
- For `productivity`, `assortment`, or `stockout`, USE COUNT() , Refer to Example 2 and 3."
- For "growth," "expansion," "grow," or "growing," respond with a specific Examples 4 and 5.
...
SELECT region, city, area, territory, distributor, route,
  COUNT(DISTINCT CASE WHEN month = 12 AND year = 2024 AND stockout = 1 THEN customer END) AS total_stockouts,
  COUNT(DISTINCT CASE WHEN month = 12 AND year = 2024 AND stockout = 0 THEN customer END) AS total_no_stockouts,
  COUNT(DISTINCT customer) AS total_shops, 
